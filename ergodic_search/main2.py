...
def run_erg_planner_on_agent(agent_map, num_freqs=100):
    ...
    traj = traj.detach().cpu().numpy()  # Assuming traj is a tensor of shape (traj_steps, 2)
...
def solve_and_evaluate(mismatch_penalty, mix_penalty, balance_penalty, variance_wt, dists, all_labels, K, N, approx_points, detail_points, wavelet, X_low_fidelity, horiz_detail, vert_detail, diag_detail, approx_coeffs, info_map, num_freqs=100):
    """
    ...
    # Initial Objective (will be updated later with additional terms)
    model += 0  # Placeholder
    
    # Constraints: Each point must be assigned to exactly one cluster
    for i in range(N):
        model += lpSum([z[i][c] for c in range(K)]) == 1
    
    # Cluster Fidelity Constraints
    ...
    # Complete Objective Function with Variance Weight
    model.setObjective(variance_wt * variance_term + mismatch_term + mix_term + balance_term)
    ...
    for traj in all_trajectories:
        for pos in traj:
            ...
            x, y = pos[0], pos[1]
            # Assuming the map spans from -5 to 5 in both x and y
...
def main():
    """
    Main function to perform parameter optimization using Optuna and plot trajectories.
    """
    ...
    def objective(trial):
        """
        Objective function for Optuna to minimize joint ergodic metric.
        """
        # Optimize penalty coefficients as floats using log-uniform distribution
        mismatch_penalty = trial.suggest_loguniform('mismatch_penalty', 1.0, 100.0)  # Adjusted range as needed
        mix_penalty = trial.suggest_loguniform('mix_penalty', 1.0, 100.0)            # Changed from suggest_int to suggest_loguniform
        balance_penalty = trial.suggest_loguniform('balance_penalty', 0.01, 10.0)    # Changed to log-uniform for consistency
        variance_wt = trial.suggest_loguniform('variance_wt', 0.1, 10.0)              # New hyperparameter for variance weighting
        num_freqs = trial.suggest_int('num_freqs', 50, 150)
        
        labels_full, avg_erg, _ = solve_and_evaluate(
            mismatch_penalty=mismatch_penalty,
            mix_penalty=mix_penalty,
            balance_penalty=balance_penalty,
            variance_wt=variance_wt,  # Pass the new variance weight
            dists=dists,
    ...
    # Step 5: Optimize Parameters with Optuna
    study = optuna.create_study(direction='minimize')
    study.optimize(objective, n_trials=50)
    
    best_params = study.best_params
    best_ergodicity = study.best_value
    
    # Step 6: Re-run with Best Parameters to Get Trajectories
    labels_full, _, trajectories = solve_and_evaluate(
        mismatch_penalty=best_params['mismatch_penalty'],
        mix_penalty=best_params['mix_penalty'],
        balance_penalty=best_params['balance_penalty'],
        variance_wt=best_params['variance_wt'],  # Pass the new variance weight
        dists=dists,
        all_labels=all_labels,
        K=K,
        N=N,
    ...
    print(f"Mismatch penalty: {best_params['mismatch_penalty']}")
    print(f"Mix penalty: {best_params['mix_penalty']}")
    print(f"Balance penalty: {best_params['balance_penalty']}")
    print(f"Variance Weight: {best_params['variance_wt']}")
    print(f"Number of Frequencies: {best_params['num_freqs']}")
    print(f"With joint ergodicity measure: {best_ergodicity}")
